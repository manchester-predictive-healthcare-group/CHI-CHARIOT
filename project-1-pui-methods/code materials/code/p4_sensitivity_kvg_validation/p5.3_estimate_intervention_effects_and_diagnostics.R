###
### Estimate the relative risk ratio for the three cohorts, which have been created in file 5.1:
###
### Full cohort
...
df_overlap_comb <- readRDS(paste("data/df_valid_overlap_comb_cohort", gender_in, "_", med, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
...
saveRDS(risk_pseudo_entire_cohort_hajek, 
        paste("data/rr_entire_cohort_hajek_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
saveRDS(risk_pseudo_overlap_cohort_hajek, 
        paste("data/rr_overlap_cohort_hajek_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
saveRDS(risk_pseudo_overlap_comb_cohort_hajek, 
        paste("data/rr_overlap_comb_cohort_hajek_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

saveRDS(risk_pseudo_entire_cohort_ht, 
        paste("data/rr_entire_cohort_ht_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
saveRDS(risk_pseudo_overlap_cohort_ht, 
        paste("data/rr_overlap_cohort_ht_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
saveRDS(risk_pseudo_overlap_comb_cohort_ht, 
        paste("data/rr_overlap_comb_cohort_ht_", med, "_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
...
run_ess_analysis <- function(df, label) {
  ...
  results <- list(
    ...
    ipacw      = compute_ess(df$ipacw_weight,         "IPACW weights only",                    df$med_status),
    ipcw_ipacw = compute_ess(df$comb_weight_censoring,          "IPCW x IPACW combined",                 df$med_status),
    iptw       = compute_ess(df$iptw_weight,          "IPTW weights only",                     df$med_status),
    combined   = compute_ess(df$comb_weight_baseline, "IPCW x IPACW x IPTW (final combined)", df$med_status)
  )
...
saveRDS(ess_results_entire_cohort, paste("data/ess_", med, "_entire_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

# Overlap cohort
ess_results_overlap_cohort <- run_ess_analysis(df_overlap, "OVERLAP COHORT")
saveRDS(ess_results_overlap_cohort, paste("data/ess_", med, "_overlap_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

# Overlap cohort comb
ess_results_overlap_cohort_comb <- run_ess_analysis(df_overlap_comb, "OVERLAP COHORT")
saveRDS(ess_results_overlap_cohort_comb, paste("data/ess_", med, "_overlap_cohort_comb_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

#################################
### Step 3: Summarise weights ###
#################################

### Compute quantiles of a weight vector, split by treatment group
### Arguments:
###   w               - vector of weights
###   med_status_vec  - vector of treatment group indicators (0/1)
###   probs           - quantiles to report (excluding min/max, which are always included)
### Returns a data.frame with one row per group
weight_quantiles <- function(w, med_status_vec, probs = c(0.5, 0.9, 0.95, 0.99, 0.999)) {
  dplyr::bind_rows(lapply(0:1, function(g) {
    w_g <- w[med_status_vec == g]
    qs  <- stats::quantile(w_g, probs = probs, na.rm = TRUE, names = FALSE)
    qs_df <- as.data.frame(t(qs))
    names(qs_df) <- paste0("p", probs * 100)
    data.frame(
      group = g,
      n     = length(w_g),
      min   = min(w_g, na.rm = TRUE),
      qs_df,
      max   = max(w_g, na.rm = TRUE),
      check.names = FALSE
    )
  }))
}

### Run weight-quantile analysis across all weighting stages for a given cohort
### Mirrors run_ess_analysis(), but reports quantiles instead of ESS
run_weight_quantile_analysis <- function(df, label, probs = c(0.5, 0.9, 0.95, 0.99, 0.999, 0.9999, 1)) {
  
  stages <- list(
    ipcw       = df$ipcw_weight,
    ipacw      = df$ipacw_weight,
    ipcw_ipacw = df$comb_weight_censoring,
    iptw       = df$iptw_weight,
    combined   = df$comb_weight_baseline
  )
  
  dplyr::bind_rows(lapply(names(stages), function(stage_name) {
    qtab <- weight_quantiles(stages[[stage_name]], df$med_status, probs = probs)
    qtab$stage <- stage_name
    qtab$label <- label
    qtab
  }))
}
...
############################################################
### Run and save weight-quantile analysis for each cohort ###
############################################################

wq_entire_cohort <- run_weight_quantile_analysis(df_valid_ipacw, "Entire cohort")
saveRDS(wq_entire_cohort,
        paste("data/weight_quantiles_", med, "_entire_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

wq_overlap_cohort <- run_weight_quantile_analysis(df_overlap, "Overlap cohort")
saveRDS(wq_overlap_cohort,
        paste("data/weight_quantiles_", med, "_overlap_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

wq_overlap_cohort_comb <- run_weight_quantile_analysis(df_overlap_comb, "Overlap cohort comb")
saveRDS(wq_overlap_cohort_comb,
        paste("data/weight_quantiles_", med, "_overlap_cohort_comb_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))

##########################################
### Step 4: FURTHER WEIGHT DIAGNOSTICS ###
##########################################
run_weight_diagnostics <- function(df, label) {
  ...
  cat("Untreated:\n")
  q_untreated <- quantile(df_untreated$comb_weight_baseline,
                          p = c(0, 0.5, 0.9, 0.95, 0.99, 0.999, 1))
  print(q_untreated)
  cat("Treated:\n")
  q_treated <- quantile(df_treated$comb_weight_baseline,
                        p = c(0, 0.5, 0.9, 0.95, 0.99, 0.999, 1))
  print(q_treated)
  
  ### 2. Weight concentration in treated group
  top1_threshold <- quantile(df_treated$comb_weight_baseline, 0.99)
  top1_share <- sum(df_treated$comb_weight_baseline[df_treated$comb_weight_baseline >= top1_threshold]) /
    sum(df_treated$comb_weight_baseline)
  ...
  weight_mat <- df_treated[, c("ipcw_weight", "ipacw_weight", "iptw_weight")]
  cor_mat <- round(cor(weight_mat, method = "spearman"), 3)
  print(cor_mat)
  
  ### 4. Profile extreme vs normal combined-weight individuals
  df_extreme_comb <- dplyr::filter(df_treated, comb_weight_baseline >= top1_threshold)
  df_normal_comb  <- dplyr::filter(df_treated, comb_weight_baseline <  top1_threshold)
  
  weight_drivers <- list(
    ipcw  = list(extreme = round(mean(df_extreme_comb$ipcw_weight), 2),
                 normal  = round(mean(df_normal_comb$ipcw_weight),  2)),
    ipacw = list(extreme = round(mean(df_extreme_comb$ipacw_weight), 2),
                 normal  = round(mean(df_normal_comb$ipacw_weight),  2)),
  ...
  df_treated$flag_ipacw <- df_treated$ipacw_weight >= quantile(df_treated$ipacw_weight, 0.95)
...
saveRDS(weight_diagnostics_entire_cohort, 
        paste("data/weight_diagnostics_", med, "_entire_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
...
saveRDS(weight_diagnostics_overlap_cohort, 
        paste("data/weight_diagnostics_", med, "_overlap_cohort_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
...
saveRDS(weight_diagnostics_overlap_cohort_comb, 
        paste("data/weight_diagnostics_", med, "_overlap_cohort_comb_", gender_in, "_tfup", t_fup_int, "_", model_flex, ".rds", sep = ""))
