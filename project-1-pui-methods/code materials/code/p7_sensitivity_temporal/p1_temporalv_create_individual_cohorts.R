...
create_df_temporalv <- function(gender, t_index){
  ...
  ### Read in the imputed validation data at baseline
  df_imp <- readRDS(paste("data/df_imp_valid_", gender, sep = ""))
  ...
  ### Reduce this to individuals who aren't censored or already had an event by t_fup
  ...
  ### Want to create a vector of variables included in the model.
  
  ### First read in variables that were interacted with the spline for age
  inter_age_rcs <- readRDS(paste("data/var_model_inter_age_rcs", gender, ".rds", sep = ""))
  
  ### Combine with age and continuous variables to get list of all variables
  var_all_model <- c("age", "IMD", "sbp", "bmi", "nonhdl", inter_age_rcs)
  
  ### Remove variables that had missingness (ethnicity, IMD, smoking status, BMI, SBP, non-HDL cholesterol),
  ### to get a list of variables with no missingness.
  var_all_model_nomiss <- var_all_model[!grepl("IMD|ethnicity|bmi|sbp|nonhdl|smoking", var_all_model)]
  
  ### For variables with no missingness, we update to the value extracted at time t_fup.
  ...
  saveRDS(df_valid, paste("data/sens_temporalv_df_valid_", gender, "_", t_fup, ".rds", sep = ""))
  print(str(df_valid))
