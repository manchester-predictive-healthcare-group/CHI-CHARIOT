...
est_calibration <- function(gender, model){
  ...
  ### Add title
  if (model == 0){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Non-causal model; t = 10 years; Estimand: Non-causal estimate", sep = ""))) 
  } else if (model == 1){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Treatment offset model; t = 10 years; Estimand: Risk under no antihypertensives", sep = ""))) 
  } else if (model == 2){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Unexposed mediator model; t = 10 years; Estimand: Risk under no antihypertensives", sep = ""))) 
  } else if (model == 3){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Modifiable risk factor model; t = 10 years; Estimand: Risk under current intervention strategy", sep = ""))) 
  } else if (model == 4){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Two-component model; Estimand: Risk under current intervention strategy", sep = ""))) 
  } else if (model == 5){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Modifiable risk factor model (SBP only); t = 10 years; Estimand: Risk under current intervention strategy", sep = ""))) 
  } else if (model == 6){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Modifiable risk factor model (BMI only); t = 10 years; Estimand: Risk under current intervention strategy", sep = ""))) 
  } else if (model == 7){
    ggplot_comb <- ggplot_comb + ggplot2::ggtitle(stringr::str_wrap(paste("Model ", model, ": Modifiable risk factor model (non-HDL cholesterol only); t = 10 years; Estimand: Risk under current intervention strategy", sep = "")))
  }
