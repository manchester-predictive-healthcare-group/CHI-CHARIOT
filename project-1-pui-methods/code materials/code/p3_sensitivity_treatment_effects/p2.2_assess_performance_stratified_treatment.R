### Assess performance within users of each treatment at baseline, vs non-users
...
stratified_med <- args[2]
stratified_type <- as.numeric(args[3])
scenario <- as.numeric(args[4])
print(paste("gender = ", gender_in))
print(paste("med = ", stratified_med))
print(paste("stratified_type = ", stratified_type))
...
### Add a final scenario where both unchanged, at the end
scenario_adjustments <- rbind(scenario_adjustments, c(1,1))

###
### Write a function that will produce a calibration curve for a given development and validation dataset
###
est_performance <- function(gender, stratified_med, stratified_type, adjustment_statins = NULL, adjustment_ah = NULL){
  ...
  ### Merge 
  df_valid <- merge(df_valid, cohort_split_times,
                    by.x = "patid",
                    by.y = "patid", 
                    all.x = TRUE)
  
  ### Reduce to those on/off treatment
  if (stratified_med == "statins"){
    df_valid <- dplyr::filter(df_valid, med_status_statins == stratified_type)
  } else if (stratified_med == "antihypertensives"){
    df_valid <- dplyr::filter(df_valid, med_status_ah == stratified_type)
  }
  
  ### Read in counterfactual survival times
  if (adjustment_statins != 1 | adjustment_ah != 1){
    print(paste("adjustment statins and adjustment ah specified", adjustment_statins, adjustment_ah))
  ...
  } else if (adjustment_statins == 1 & adjustment_ah == 1){
    print(paste("stratified analysis with no-miss specification of treatment effect"))
  ...
  saveRDS(df_calib_smooth, paste("data/sens_treatment_effect_modified_calib_ph_df_smooth_", gender,  
                                 "_", stratified_med, stratified_type,
                                 "_statins", adjustment_statins, "_ah", adjustment_ah, ".rds", sep = ""))
